...
export const INTERACTION_RULES:InteractionRule[]=[
  ...
  {id:"fire-pops-balloon",source:{tags:["fire-source"]},target:{tags:["balloon"]},trigger:"collision",effect:"pop",description:"Direkter Flammenkontakt zerstört die Ballonhülle.",signal:"balloon.popped"},
  ...
  {id:"fish-attracts-cat",source:{tags:["cat-attractor"]},target:{tags:["cat"]},trigger:"proximity",effect:"chase",description:"Joanne läuft zu einem sichtbaren, zappelnden Mr. Blue.",maxVerticalDistance:70,maxDistance:700,direction:"opposite-target",signal:"cat.chasing"},
  {id:"gear-mesh",source:{tags:["gear"]},target:{tags:["gear"]},trigger:"proximity",effect:"transfer-rotation",description:"Zahnräder im passenden Achsabstand drehen sich entgegengesetzt.",minDistance:70,maxDistance:98,direction:"clockwise-inverted",signal:"gear.connected"},
...
const matches=(definition:GadgetDefinition,selector:GadgetSelector)=>
...
export function resolveInteractions(sourceType:GadgetType,targetType:GadgetType,trigger:InteractionRule["trigger"],kinematics:InteractionKinematics):ResolvedInteraction[]{
  ...
  for(const rule of INTERACTION_RULES){
    ...
    if(matches(source,rule.source)&&matches(target,rule.target)&&conditionMatches(rule,kinematics))resolved.push({rule,source,target,reversed:false});
    else if(matches(target,rule.source)&&matches(source,rule.target)&&conditionMatches(rule,{...kinematics,sourceX:kinematics.targetX,sourceY:kinematics.targetY,targetX:kinematics.sourceX,targetY:kinematics.sourceY,relativeVelocity:{x:-kinematics.relativeVelocity.x,y:-kinematics.relativeVelocity.y}}))resolved.push({rule,source:target,target:source,reversed:true});
  }
  return resolved;
}

export const interactionRows=()=>INTERACTION_RULES.map(rule=>({source:Object.values(GADGET_CATALOG).filter(gadget=>matches(gadget,rule.source)).map(gadget=>gadget.displayName).join(", "),target:Object.values(GADGET_CATALOG).filter(gadget=>matches(gadget,rule.target)).map(gadget=>gadget.displayName).join(", "),trigger:rule.trigger,effect:rule.description,status:"aktiv" as const}));
